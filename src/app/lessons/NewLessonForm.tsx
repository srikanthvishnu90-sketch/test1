...
import { QUESTION_DESIGN_PRINCIPLES } from "@/domain/intelligence/questionDesign";
...
function SurveyPreviewCard({
  ...
  return (
    <div className="mt-6 overflow-hidden rounded-card border border-ink-wash bg-paper">
      ...
      <details className="border-t border-ink-wash px-4 py-3">
        <summary className="cursor-pointer text-[12px] font-medium text-ink-tint">
          Why these questions look this way — grounded in research
        </summary>
        <ul className="mt-3 flex flex-col gap-2">
          {QUESTION_DESIGN_PRINCIPLES.map((p) => (
            <li key={p.id} className="text-[13px] leading-snug">
              <span className="text-ink-black">{p.rule}</span>{" "}
              <span className="text-secondary">— {p.because}</span>{" "}
              <span className="text-[11px] italic text-secondary">({p.cite})</span>
            </li>
          ))}
        </ul>
        <p className="mt-3 text-[11px] text-secondary">
          Tuned for high schoolers (ages 14–18). See docs/question-design.md.
        </p>
      </details>
    </div>
  );
