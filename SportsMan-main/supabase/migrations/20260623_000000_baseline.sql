...
--
-- Idempotent: create-if-not-exists tables/indexes/columns, drop+create policies,
-- create-or-replace functions, drop+create triggers. Safe to re-run on a fresh
-- database to reconstruct the schema from source.
--
-- SECURITY MODEL (unchanged):
--   • RLS enabled on EVERY public table → default-deny; a row is reachable only
--     if a policy allows it for the caller's role (anon / authenticated).
--   • GRANTs govern API exposure (what PostgREST attempts); RLS governs WHICH
--     rows. Both are required.
--   • service_role BYPASSES RLS — server-side only (Edge Functions), never the
--     client. The client uses the anon/publishable key under these policies.
--   • athletes are MINORS — parent-only; providers see a minimal display
--     (first name + age band) denormalized onto bookings, never the table.
-- ============================================================================
...
-- providers: public-safe business identity owned by a provider profile.
...
-- NOTE: session_id is NULLABLE with ON DELETE SET NULL (live state) so a booking
-- survives deletion of its session; athlete/program FKs likewise SET NULL.
...
-- session_id was tightened from NOT NULL/CASCADE to NULLABLE/SET NULL on live.
-- drop-not-null is a no-op if already nullable. (The FK on-delete action is
-- SET NULL as created above on a fresh DB.)
...
create index if not exists idx_athletes_parent        on public.athletes (parent_id);
create index if not exists idx_bookings_searcher      on public.bookings (searcher_id);
create index if not exists idx_bookings_session       on public.bookings (session_id);
create index if not exists idx_conversations_searcher on public.conversations (searcher_id);
...
drop policy if exists programs_select_owner on public.programs;
...
drop policy if exists athletes_delete_parent on public.athletes;
...
drop policy if exists bookings_select_searcher on public.bookings;
...
drop policy if exists bookings_insert_searcher on public.bookings;
create policy bookings_insert_searcher on public.bookings
  for insert to authenticated with check (searcher_id = auth.uid());
drop policy if exists bookings_update_searcher on public.bookings;
...
create policy bookings_update_provider on public.bookings
  ...
  with check (
    exists (select 1 from public.sessions s
            ...
            join public.providers pv on pv.id = pr.provider_id
            where s.id = bookings.session_id and pv.owner_id = auth.uid()));
drop policy if exists bookings_delete_searcher on public.bookings;
...
drop policy if exists teams_select_parent on public.teams;
...
drop policy if exists team_athletes_select_parent on public.team_athletes;
...
-- handle_new_user(): auto-provision a profiles row on signup. SECURITY DEFINER
-- so it can insert past RLS (the new user has no session at insert time).
-- search_path pinned to '' (all object refs are schema-qualified) to prevent
-- search-path hijacking. EXECUTE is revoked from client roles (only the auth
-- trigger fires it).
...
-- enforce_booking_provider_update(): RLS can't restrict WHICH columns the
-- provider updates, so this pins a provider's UPDATE to `status` only. The
-- searcher (owner) is unaffected. search_path pinned to ''; EXECUTE revoked.
