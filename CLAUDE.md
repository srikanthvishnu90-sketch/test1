...
- QUESTION DESIGN = how we formulate any student-facing question is governed by
  `docs/question-design.md` (developmental + survey-methods research, high-school
  focus) and enforced by `src/domain/intelligence/questionDesign.ts` (`reviewQuestion`
  flags leading/double-barreled/abstract-trait/yes-no/too-long items; principles +
  age-band formats are exported for surfacing). Self-report is a divergence signal,
  never ground truth.
